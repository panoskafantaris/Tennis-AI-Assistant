"""
Ball tracker — manages the ball's state across frames.

Responsibilities:
  - Maintain a rolling buffer of detections (trajectory / trail)
  - Handle missing detections (ball not visible)
  - Provide clean interface: update(detection) → BallState
"""
...
class BallState:
    ...
    missing    : int   = 0          # consecutive frames without detection
...
class BallTracker:
    ...
    Logic:
      ...
      - On miss: increment missing; if > max_missing → clear trail
    """
    ...
    def __init__(self):
        ...
        self._missing : int   = 0
        self._state   : BallState = BallState()
    ...
    def update(
        ...
        """
        detection : (x, y, confidence) from inference, or None
        returns   : current BallState
        """
